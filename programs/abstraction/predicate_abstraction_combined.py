...
class PredicateAbstraction:
    ...
    def combine_program_transitions(self):
        new_con_env_trans = []
        ...
        abstract_guard_con = {}
        for con_trans in self.program.con_transitions:
            state_con_to[con_trans.tgt].add(con_trans)
            abstract_guard_con[con_trans] = self.abstract_guard(con_trans.condition, self.program.con_events)

        abstract_guard_env = {}
        for env_trans in self.program.env_transitions:
            state_env_from[env_trans.src].add(env_trans)
            abstract_guard_env[env_trans] = self.abstract_guard(env_trans.condition, self.program.env_events)

        for s in state_con_to.keys():
            all_possible_prev_con = state_con_to[s]
            all_possible_next_env = state_env_from[s]
            for con_trans in all_possible_prev_con:
                for env_trans in all_possible_next_env:
                    new_cond = con_trans.condition
                    new_cond = conjunct(new_cond, env_trans.condition.replace(con_trans.action))
                    new_action = [BiOp(act.left, ":=", act.right.replace(con_trans.action)) for act in env_trans.action]
                    new_action += [act for act in con_trans.action if not any(a for a in env_trans.action if a.left == act.left)]
                    new_con_env_trans.append((abstract_guard_con[con_trans], abstract_guard_env[env_trans], Transition(con_trans.src,
                                                        new_cond,
                                                        new_action,
                                                        env_trans.output + [neg(o) for o in self.program.out_events if o not in env_trans.output],
                                                        env_trans.tgt)))

        init_trans = []

        init_conf = conjunct_typed_valuation_set(self.program.valuation)
        for t in self.program.env_transitions:
            if t.src == self.program.initial_state:
                if smt_checker.check(And(*conjunct(t.condition, init_conf).to_smt(self.program.symbol_table))):
                    abstract_guard = self.abstract_guard(conjunct(init_conf, t.condition), self.program.env_events)
                    init_trans += [(true(), abstract_guard, Transition(t.src,
                                             t.condition,
                                             t.action,
                                             t.output + [neg(o) for o in self.program.out_events if o not in t.output],
                                             t.tgt))]
        return init_trans, new_con_env_trans


    def abstract_guard(self, guard: Formula, events):
        ...
        satisfying_behaviour = [conjunct_formula_set(E) for E in powerset if
                                    smt_checker.check(And(*conjunct_formula_set(E | {guard}).to_smt(self.program.symbol_table)))]

        return disjunct_formula_set(satisfying_behaviour)
    ...
    def initialise(self, debug=True):
        ...
        init_transitions, con_env_transitions = self.combine_program_transitions()
        ...
        self.program_init_trans = init_transitions
        self.program_con_env_trans = con_env_transitions

        self.all_program_trans = init_transitions + con_env_transitions

        self.transition_tags = {transition_formula(t):{frozenset(): frozenset({frozenset():frozenset()})} for _, _, t in self.all_program_trans}
        self.con_tags = {transition_formula(t):{frozenset(): frozenset({frozenset():frozenset()})} for _, _, t in self.all_program_trans}

        self.pred_transition_invars = {transition_formula(t):{} for _, _, t in self.all_program_trans}
        self.con_pred_invars = {transition_formula(t):{} for _, _, t in self.all_program_trans}

        # self.abstraction = Program("pred_abst_" + program.name, states | {init_st}, init_st, self.program.valuation,
        #                            init_transitions, con_env_transitions, program.env_events,
        ...
        init_transition_ltl = disjunct_formula_set(
            [conjunct_formula_set([abstract_guard, Variable(t.tgt)] + t.output)
             for _, abstract_guard, t in init_transitions])

        transition_ltl = []
        for (abstract_guard_con, abstract_guard_env, t) in con_env_transitions:
            now = conjunct_formula_set([Variable(t.src), abstract_guard_con])
            next = conjunct_formula_set([abstract_guard_env, Variable(t.tgt)] + t.output)

            transition_ltl += [conjunct(now, X(next))]
    ...
    def add_predicates(self, new_state_predicates: [Formula], new_transition_predicates: [Formula], pred_name_dict : dict, simplified: bool):
        ...
        for p in new_state_predicates + new_transition_predicates:
            self.transition_tags, self.pred_transition_invars = self.tag_transitions(self.transition_tags, p)
        ...
        #     predicate_effect = self.transition_tags[t_formula]
        #
        ...
        init_transitions = []
        for _, abstract_guard, t in self.program_init_trans:
            t_formula = transition_formula(t)

            effects = []
            if t_formula in self.pred_transition_invars.keys():
                effects += [pred_name_dict[p] for p in self.pred_transition_invars[t_formula] if p in init_preds]
                effects += [neg(pred_name_dict[p]) for p in self.pred_transition_invars[t_formula] if neg(p) in init_preds]

            if t_formula in self.transition_tags.keys():
                predicate_effect = self.transition_tags[t_formula]
        ...
        transition_ltl = []
        for C, E, t in self.all_program_trans:
            t_formula = transition_formula(t)
            effects = []
            if t_formula in self.pred_transition_invars.keys():
                effects += [iff(p, X(p)) for p in self.pred_transition_invars[t_formula]]

            if t_formula in self.transition_tags.keys():
                predicate_effect = self.transition_tags[t_formula]
                transition_predicate_effect = [conjunct(conjunct_formula_set([pred_name_dict[p] for p in now]),
                                                        X(disjunct_formula_set([conjunct_formula_set([pred_name_dict[p] for p in next]) for next in nexts])))
                                                              for now, nexts in predicate_effect.items()]

                effects += [disjunct_formula_set(transition_predicate_effect)]
            transition_ltl += [conjunct_formula_set([Variable(t.src)] + C + effects + [X(conjunct_formula_set([Variable(t.tgt), E] + t.output))])]
